        ...
        {masters.map((master) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex items-center gap-5 sm:gap-3 rounded-xl sm:rounded-lg border-2 p-5 sm:p-3.5 text-left transition-all",
            ...
            >
              <div
                className={cn(
                  "flex h-14 w-14 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-full text-xl sm:text-sm font-medium text-white",
              ...
              <div className="flex-1">
                <p className="font-semibold text-[19px] sm:text-sm">{master.name}</p>
                <p className="text-[15px] sm:text-xs text-muted-foreground">
                  {selected ? "Выбран" : "Нажмите, чтобы выбрать"}
                </p>
              </div>
              {selected && (
                <div className="flex h-7 w-7 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-foreground">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 12 12"
